...
internal sealed class AzureSchedulerBehaviorHarness : ISchedulerBehaviorHarness
{
    private AzurePlatformTestContext? context;

    public ISchedulerClient SchedulerClient => context?.SchedulerClient ?? throw new InvalidOperationException("Harness has not been initialized.");

    public ISchedulerStore SchedulerStore => context?.SchedulerStore ?? throw new InvalidOperationException("Harness has not been initialized.");
    ...
    public async ValueTask InitializeAsync()
    {
        context = await AzurePlatformTestContext.CreateAsync(
                nameof(AzureSchedulerBehaviorHarness),
